...
Solution.prototype.shuffle = function () {
  ...
  for (let i = 0; i < arr.length; i++) {
    const index = randomInt(i, arr.length - 1);
...
- 如果额外存储或直接 return 循环中的指针值, 使用 `left<=right`;
  - 此时区间为 [right+1,right], 区间不存在元素;
  - 推荐使用该方法;
- 反之使用 `left < right`, 跳出循环后返回 left 或 right;
  - 此时区间为 [right,right], 区间内存在一个元素;
  - 此元素为二分查找最终结果, 但不能确定该元素是否为查找值;
...
- 判断 arr[m] 与 target 的关系;
  ...
  - 若 arr[m] = target, 返回 m;
- 循环结束条件为 start > end;
- 若 target 不存在, 二分查找结束时, start 指向首个大于 target 的元素, end 指向首个小于 target 的元素;
